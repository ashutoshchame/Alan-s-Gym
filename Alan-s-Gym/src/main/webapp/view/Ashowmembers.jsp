...
<nav class="navbar navbar-expand-lg bg-body-tertiary">
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="AdminHomepagebar">Home</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-black"  href="AdminShowmemberspagebar">Show Members</a>
        </li>
       
         <li class="nav-item">
          <a class="nav-link text-black" href="AdminLogoutpagebar">Logout</a>
        </li>
      </ul>
    </div>
  </div>
...
<centre><h2>Gym Members Table</h2></centre>

<center><table class="border border-3 border-dark">
  <tr class="border border-3 border-dark">
     <th class="border border-3 border-dark">Username</th>
     <th class="border border-3 border-dark">Name</th>
    <th class="border border-3 border-dark">Age</th>
    <th class="border border-3 border-dark">City</th>
     <th class="border border-3 border-dark">Mobile No.</th>
     <th class="border border-3 border-dark">Password</th>
  </tr>
  
 
   <%			
		Class.forName("com.mysql.jdbc.Driver");
		Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/Alansgym", "root", "root");
		PreparedStatement ps = connection.prepareStatement("select * from userlogin");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			out.println("<tr><td>" + rs.getString(1) + "</td><td>" + rs.getString(5) + "</td><td>"+ rs.getString(2)+"</td><td>" + rs.getString(3) + "</td><td>" + rs.getString(4) + "</td><td>" + rs.getString(6) + "</td></tr>");
		}
		%>
